 ...
 */
function get_code($type,$length)
{
    if(!in_array($type,['mix','num','zh'])){
        return false;
    }else{
        $config = array(
            'mix'   =>  '2345678abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXY', // 验证码字符集合
            'num'    =>  '1234567890',            // 验证码过期时间（s）
        ...
        for ($i = 0; $i<$length; $i++) {
            $code[$i] = $config[$type][mt_rand(0, strlen($config[$type])-1)];
        }
        // 保存验证码
